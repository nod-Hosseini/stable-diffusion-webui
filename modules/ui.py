        ...
        with gr.Row().style(equal_height=False):
            with gr.Accordion("Open for Settings"):
                with gr.Column(variant='panel', elem_id="txt2img_settings"):
                    steps = gr.Slider(minimum=1, maximum=150, step=2, label="Sampling Steps", value=20)
                    sampler_index = gr.Radio(label='Sampling method', elem_id="txt2img_sampling", choices=[x.name for x in samplers], value=samplers[0].name, type="index")

                    with gr.Group():
                        width = gr.Slider(minimum=64, maximum=2048, step=32, label="Width", value=512)
                        height = gr.Slider(minimum=64, maximum=2048, step=32, label="Height", value=512)
                    ...
                    with gr.Row(visible=False) as hr_options:
                        firstphase_width = gr.Slider(minimum=0, maximum=1024, step=32, label="Firstpass width", value=0)
                        firstphase_height = gr.Slider(minimum=0, maximum=1024, step=32, label="Firstpass height", value=0)
                        denoising_strength = gr.Slider(minimum=0.0, maximum=1.0, step=0.01, label='Denoising strength', value=0.7)

                    with gr.Row(equal_height=True):
                        batch_count = gr.Slider(minimum=1, maximum=8, step=1, label='Batch count', value=4)
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with gr.Row().style(equal_height=False):
            with gr.Column(variant='panel', elem_id="img2img_settings"):
                ...
                with gr.Group():
                    width = gr.Slider(minimum=64, maximum=2048, step=32, label="Width", value=512, elem_id="img2img_width")
                    height = gr.Slider(minimum=64, maximum=2048, step=32, label="Height", value=512, elem_id="img2img_height")
